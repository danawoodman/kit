...
This function allows you to modify (or replace) a `fetch` request that happens inside a `+page.js` `load` function that runs only on the server (or during pre-rendering).
